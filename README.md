...
| `malformed_parity` | which bad configuration values fail the load | 29 / 29 |
...
Six of them exist because the others could not reach what they cover. A
harness that drives whole queries through real fonts sees only what a font
set happens to provoke: nothing in a normal corpus carries a charset test, a
range on both sides, or a conditional `<alias>`; nothing is malformed enough
to send the scanner down a fallback chain. And three whole questions are
invisible to *any* comparison of properties -- how firmly a matched font
holds each one, whether a configuration loaded at all or the built-in
fallback did, and what a font name means before matching starts. Every one
of those was wrong while every harness was green. So `compare_parity` asks
`fc-pattern -c` and this crate the same question one operator at a time and
needs no fonts, `fallback_parity` builds 23 deliberately broken ones,
`bind_parity` compares nothing but the `(s)`/`(w)` marks `fc-match -v`
prints, `parse_parity` reads 72 names before substitution, and
`include_parity` and `malformed_parity` compare whether a file was accepted
rather than what it said. See `docs/audit.md` for the five audit logs.
