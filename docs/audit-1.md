# First audit: 25 findings

Carried out independently, by agents belonging to someone other than this
crate's author. It pinned typordo at `cf45c33` and compared against fontconfig
**2.18.3**, while this crate targets **2.17.0** deliberately — so some findings
are gaps and some are version drift, and telling those apart is the first thing
each entry does.

One entry here, 9.5, was marked fixed and was not; the second audit caught
that. See [audit-2.md](audit-2.md).
